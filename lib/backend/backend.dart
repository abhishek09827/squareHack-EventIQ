...
Future maybeCreateUser(User user) async {
  final userRecord = BusinessRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await BusinessRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createBusinessRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      BusinessRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createBusinessRecordData(email: email));
}
